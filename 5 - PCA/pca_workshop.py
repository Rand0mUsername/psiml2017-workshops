...
# introduce random noise generated from the picture
count = 0
...
# normalize
samples_minus_mean = all_samples - mean_vector

print(all_samples.shape)

cov_mat = np.zeros((3,3))
cov_mat = (np.matmul(samples_minus_mean, samples_minus_mean.T)) / (N - 1.0)
...
eig_pairs = list(zip(eig_val_cov, eig_vec_cov.T))
print(eig_pairs)
# eig_pairs.append((-123, 123)) # test
eig_pairs.sort(key = lambda x: np.absolute(x[0]), reverse=True)
...
eigvec1 = eig_pairs[0][1].reshape(3, 1)
eigvec2 = eig_pairs[1][1].reshape(3, 1)

matrix_w = np.hstack((eigvec1, eigvec2))
print('Matrix W:\n', matrix_w)
...
transformed = np.matmul(matrix_w.T, all_samples)
