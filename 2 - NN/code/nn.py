...
class IdentityActivation(object):
    ...
    def forward(self, preactivation):
        ...
        return numpy.copy(preactivation)
...
class SoftmaxWithCrossEntropyLayer(object):
    ...
    def forward(self, x_input):
        ...
        xexp = numpy.exp(x_input)
        self.y_output = xexp / numpy.sum(xexp)
    ...
    def backward(self, target):
        ...
        self.input_gradients = self.y_output - target
...
class FullyConnectedLayer(object):
    ...
    def __init__(self, rng, inputs_count, outputs_count, activation_type):
        ...
        self.w_weights = numpy.asarray(
            rng.uniform(
                ...
                size=(outputs_count, inputs_count) # R x C
            ),
            dtype=numpy.float
        )
    ...
    def forward(self, x_input):
        ...
        preactivation = numpy.dot(self.w_weights, self.x_input) + self.b_biases # outxin * inx1 = outx1
    ...
    def backward(self, output_grad):
        ...
        pre_activation_gradient = self.activation.backward(self.y_output, output_grad) # outx1
        ...
        self.weight_gradients = numpy.dot(pre_activation_gradient, self.x_input.T) #outx1 puta 1xin = outxin
        self.bias_gradients = numpy.copy(pre_activation_gradient)
        self.input_gradients = numpy.dot(self.w_weights.T, pre_activation_gradient) # inxout puta outx1 = in x 1
...
def create_one_hot(target, num_classes):
    one_hot = numpy.zeros((num_classes, 1), dtype=numpy.float)
    one_hot[target] = 1
    return one_hot
...
def train_nn_with_sgd(dataset_path, epochs_count, alpha):
    ...
    for epoch in range(epochs_count):
        ...
        for i in range(train_input.shape[0]):
            ...
            one_hot = create_one_hot(train_target[i], num_classes)
            neural_net.backward(one_hot)
